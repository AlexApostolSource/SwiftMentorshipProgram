...
- The fix section resolves all three pitfalls, and each one now uses a genuinely different technique: the caching actor records the in-progress work as a `Task` before the first `await`, so a reentrant call shares that result instead of racing its own; the price cache holds a lock around the entire method via a small actor-based mutex, trading away some concurrency for correctness; and the `Player` fix commits the whole outcome synchronously before the only `await`, so there's nothing left to race. None of the three is a restatement of another, so I'm fairly confident all three earn a full worked example rather than a shorter pointer back to an earlier one.
